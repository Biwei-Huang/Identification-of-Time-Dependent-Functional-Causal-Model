function [nlZ dnlZ posterior_mean posterior_covariance] = infExact_ins(hyp, mean, cov, lik, T,N,sign,x,y,DX)
...
m_extend=ones(N*(N-1)/2*T_train,1)*m(1);
m_all=DX*m_extend;   % the mean of the marginal likelihood

K_extend=kron(K,eye(N*(N-1)/2));  %kronecker product
...
if nargout>1                               % do we want the marginal likelihood?
  ...
  if nargout>=2                                         % do we want derivatives?
    ...
    for i = 1:numel(hyp.cov)
        dnlZ.cov(i) = sum(sum(Q.*(DX*kron(feval(cov{:}, hyp.cov, x, [], i),eye(N*(N-1)/2))*DX')))/2;
    end
    dnlZ.lik = sn2*trace(Q);
    for i = 1:numel(hyp.mean), 
        tmp=feval(mean{:}, hyp.mean, x, i);
        dnlZ.mean(i) = -(DX*ones(N*(N-1)/2*T_train,1)*tmp(1))'*alpha;
    end
  end
end
